...
* Настройки
  - PipeLine > Timeout, minutes - если за это время миграция не прекратилась - поставить статус `failure`
  - PipeLine > Quick Run Timeout, seconds - задержка между созданием миграции и её запуском, чтобы избежать ситуации, когда 1С решила что всё уже отработало и пришла с новым обменом.
* Импорт
  - ***[?] TODO:*** при переводе миграции в failure запускать `drush mrc`
* Привязка картинок
* Привязка вариаций
* Защита от долгих обменов
  - 
...
* Копируем плагины Миграций, переобзываем их и пишем свою логику
...
## Траблшутинг
* Чаще всего 1С не умеет общаться по https (в наших случаях это 100%), для этого стоит для сайта сделать технический адрес с HTTP, который не будет делать редирект на HTTPS
